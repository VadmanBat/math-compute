...
namespace nrcki {
class DebounceOff final : public Block {
...
public:
    explicit DebounceOff(const Context& context, const Type& T) :
        Block(context),
    ...
    void compute() const override {
        if (prev_x != (*ports->inputs[0] != 0)) {
            T_off  = prev_x ? context.time + T : std::numeric_limits<Time>::max();
    ...
    types::string printSource() const override {
        ...
        line << "    " << var_T_off << " = " << var_prev_x << " ? time + " << T << " : " << max << ";\n";
        line << "    " << var_prev_x << " = " << x << " != 0;\n";
        line << "}\n";
